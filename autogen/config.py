...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "exported.json")) as f:
...
mod_installed = set([os.path.basename(x) for x in glob.glob(os.path.join(stellaris_path, "*"))])
mod_loaded = set([str(i['SteamId']) for i in mod_load_list_json["ModIds"] if i['SteamId']])
