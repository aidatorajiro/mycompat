...
# Mod order in case of job definition conflict. Rightmost jobid will be used in case of conflict.
mod_order = ['1311725711', '2785989313']
...
def all_jobs():
    ...
    for modid, job_defs in job_def_table.items():
        for seg in job_defs:
            ...
            if len(job_modids) > 1:
                if modid != max([(mod_order.index(x), x) for x in job_modids], key=lambda x: x[0])[1]:
                            ...
                            match x[0]:
                                ...
                                case b'produces' | b'upkeep':
                                    if b'multiplier' in x[2]:
                                        ...
                                        # TODO: implement workaround for unsupported things
    ...
    with open(patchpath("common/script_values/蛾自動生成all_jobs_patch.txt"), 'wb') as f:
        f.write(sv_output)
